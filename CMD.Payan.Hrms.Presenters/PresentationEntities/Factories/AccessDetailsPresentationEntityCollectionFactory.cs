...
{
    public class AccessDetailsPresentationEntityCollectionFactory : PresentationEntityCollectionFactory
    {
        ...
        public static IPresentationEntityCollectionFactory Factory
        {
            get
            {
                if (_factory == null)
                {
                    _factory = new AccessDetailsPresentationEntityCollectionFactory();
                }
        ...
        {
            return new AccessDetailsPresentationEntityCollection(presenter, businessObjectCollection);
        }

        protected override IPresentationEntityCollection Create(IPresenter presenter)
        {
            return new AccessDetailsPresentationEntityCollection(presenter);
        }

        protected override IPresentationEntityCollection Create(IPresenter presenter, IParent parent)
        {
            return new AccessDetailsPresentationEntityCollection(presenter, parent);
        }

        protected override IPresentationEntityCollection Create(IObserver observer, IPresenter presenter, IParent parent)
        {
            return new AccessDetailsPresentationEntityCollection(observer, presenter, parent);
        }
    }
}
