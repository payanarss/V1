using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Drawing;
using System.Data;
using System.Linq;
using System.Text;
using Payanar.SS.Net.Libraries.Views.CustomControls;
using Payanar.SS.Net.Libraries.Common;
using Payanar.SS.Net.Libraries.PresentationLayer.Interfaces;
using CMD.Payan.Hrms.Presenters;

namespace CMD.Payan.Hrms.Views
{
    public partial class TableDetailView : Payanar.SS.Net.Libraries.Views.CustomControls.BaseView, ITableDetailView, IView
    {
        #region Constructors

        public TableDetailView()
            : base(TableDetailPresenterFactory.Factory)
        {
            InitializeComponent();
            Title = "TableDetail";
        }

        public TableDetailView(IObserver observer, IPresentationEntity presentationEntity)
            : base(observer, TableDetailPresenterFactory.Factory, presentationEntity)
        {
            InitializeComponent();
            Title = "TableDetail";
        }

        #endregion Constructors

        #region Properties

        public TableDetailPresentationEntity TableDetailPresentationEntityDetail
        {
            get { return PresentationEntityDetail as TableDetailPresentationEntity; }
        }

        public TableDetailPresenter TableDetailPresenterDetail
        {
            get { return PresenterDetail as TableDetailPresenter; }
        }

        #endregion Properties

        #region Methods

        public override void UpdateObserver(object value)
        {
            base.UpdateObserver(value);
        }

        public override void LoadDatas()
        {
            TableDetailPresentationEntityDetail.Load();
            base.LoadDatas();
        }

        #endregion Methods
    }
}
